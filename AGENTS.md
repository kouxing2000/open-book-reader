...
> private `~/.claude/` config — never in `AGENTS.md`, `CLAUDE.md`, `README.md`, `SECURITY.md`, `store/`, `src/`,
...
>
> **Issues, PRs, and their comments are the same public surface — and a REPORTER'S EMAIL never
> goes there.** A feedback issue filed here carries no reporter address in any form: not in an
> `<!-- reporter-email -->` marker, not inside a raw-metadata JSON block, not in a "notified X"
> comment. It stays in the gitignored `.feedback/` copy, which is what the reply tooling reads.
> An HTML comment is invisible when rendered and plain in the page source, the API, and the
> issue's **edit history** — and no API can delete a history revision, so a redaction after the
> fact leaves the original one click away. Publishing a third party's address is effectively
> permanent; treat it as unsendable, not as fixable later.
